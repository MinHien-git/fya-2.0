...
export default function ProposalsAccepted() {
  const [proposalsAccepted, setProposalsAccepted] = useState<any>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 15;

  const [showProposalsAcceptedPopup, setShowProposalsAcceptedPopup] = useState(false);
  const [selectedProposalsAccepted, setSelectedProposalsAccepted] = useState<any>('');

  const toggleProposalsAcceptedPopup = (proposalsAccepted) => {
    setSelectedProposalsAccepted(proposalsAccepted);
    setShowProposalsAcceptedPopup(!showProposalsAcceptedPopup);
  };
  
  const fetchProposalsAccepted = async () => {
    const result = await GetProposalsAccepted();
    if (result.data.data) {
      setProposalsAccepted(result.data.data);
    }
  };

  useEffect(() => {
    fetchProposalsAccepted();

    const intervalId = setInterval(fetchProposalsAccepted, 10000);
  ...
  const currentItems = proposalsAccepted.slice(startIndex, startIndex + itemsPerPage);
    ...
    <main className="w-full min-h-80 py-2 pt-0 flex flex-col gap-2 overflow-y-auto h-full mx-auto">
      <div className="w-full">
        <div className="flex justify-center items-center h-[4rem] border-b-2 gap-4">
          ...
          <p>Proposals Accepted by agency to client's briefing</p>
        </div>
      </div>
      <div className="table-container overflow-x-scroll overflow-y-scroll">
            ...
            {currentItems.map(
              ({
                ...
                >
                  ...
                  <Table.Cell className="border text-xs truncate"><span className="cursor-pointer text-blue underline" onClick={() => toggleProposalsAcceptedPopup({
                    company_name,
                    address,
                    team_members,
                    page_id,
        ...
        <Popup show={showProposalsAcceptedPopup} onClose={toggleProposalsAcceptedPopup}>
          ...
          <section className="w-full mx-auto px-2 flex mt-1 gap-2">          
            <div className="grid gap-2">
              <div className="max-w-[22rem] rounded-xl border-2 flex-col pl-8 pr-16 py-4">
                <h2 className="text-lg font-bold mb-2">{selectedProposalsAccepted.fname} {selectedProposalsAccepted.lname} company detail:</h2>
                <p className="text-sm mb-1"><strong>Company Name:</strong> {selectedProposalsAccepted.companyName}</p>
                <p className="text-sm mb-1"><strong>Client's Name:</strong> {selectedProposalsAccepted.fname} {selectedProposalsAccepted.lname}</p>
                <p className="text-sm mb-1"><strong>Client's role in the company:</strong></p>
                <p className="text-sm mb-1"><strong>Office address:</strong> {selectedProposalsAccepted.companyLocation}</p>
                <p className="text-sm mb-1"><strong>Team size:</strong> {selectedProposalsAccepted.companySize}</p>
                <p className="text-sm mb-1"><strong>Industry:</strong> {selectedProposalsAccepted.industry}</p>
                <p className="text-sm mb-1"><strong>Rating:</strong> {selectedProposalsAccepted.rating}</p>
              </div>
              <div className="max-w-[22rem] rounded-xl border-2 flex-col pl-8 pr-16 py-4">
                <h2 className="text-lg font-bold mb-2">Agency detail:</h2>
                <p className="text-sm mb-1"><strong>Name:</strong> {selectedProposalsAccepted.company_name}</p>
                <p className="text-sm mb-1"><strong>Office address:</strong> {selectedProposalsAccepted.address}</p>
                <p className="text-sm mb-1"><strong>Team size:</strong> {selectedProposalsAccepted.team_members}</p>
                <p className="text-sm mb-1"><strong>Rating:</strong> {selectedProposalsAccepted.prating}</p>
              </div>
            </div>
            <div className="rounded-xl border-2 flex-col px-8 py-4">
              <h2 className="text-lg font-bold mb-2">{selectedProposalsAccepted.company_name} proposal for the project:</h2>
              <h2 className="text-lg font-bold mb-2">{selectedProposalsAccepted.projectTitle}</h2>
              <p className="text-sm mb-1"><strong>Services wanted:</strong></p>
              {selectedProposalsAccepted.services && (
                <div className="grid gap-2 border-2 rounded-xl p-0.5">
                  <div className="flex">
                    {selectedProposalsAccepted.services.slice(0, 3).map((service, index) => (
                      <div key={index} className="text-sm mx-0.5 py-0.5 px-6 rounded-lg bg-blue text-white">{service}</div>
                    ))}
                    {selectedProposalsAccepted.services.length > 3 && (
                      <div>{`+${selectedProposalsAccepted.services.length - 3} more`}</div>
                    )}
                  </div>
                </div>
              )}
              <p className="text-sm mb-1"><strong>Skills required:</strong></p>
              {selectedProposalsAccepted.skills && (
                <div className="grid gap-2 border-2 rounded-xl p-0.5">
                  <div className="flex">
                    {selectedProposalsAccepted.skills.slice(0, 3).map((skill, index) => (
                      <div key={index} className="text-sm mx-0.5 py-0.5 px-6 rounded-lg bg-secondary">{skill}</div>
                    ))}
                    {selectedProposalsAccepted.skills.length > 3 && (
                      <div>{`+${selectedProposalsAccepted.skills.length - 3} more`}</div>
                    )}
                  </div>
                </div>
              )}
              <div className="grid gap-2 mt-2 max-w-[35rem]">
                <div className="flex flex-col lg:flex-row lg:justify-between">
                  <div className="lg:w-1/2 mr-1">
                    <p className="text-sm mb-1"><strong>Price:</strong> {selectedProposalsAccepted.price}</p>
                    <p className="text-sm mb-1"><strong>Duration:</strong> {selectedProposalsAccepted.duration}</p>
                  </div>
                  <div className="lg:w-1/2">
                    <p className="text-sm mb-1"><strong>Budget range:</strong> {selectedProposalsAccepted.budgetRange}</p>
                    <p className="text-sm mb-1"><strong>Client's email:</strong> {selectedProposalsAccepted.email}</p>
                  </div>
                </div>
                <p className="text-sm mb-1"><strong>Proposal:</strong></p>
                <p className="text-sm mb-1">{selectedProposalsAccepted.description}</p>
              </div>
            </div>
          </section>
        </Popup>
      ...
      <div className="flex justify-between mt-4">
        ...
        <button
          ...
          disabled={startIndex + itemsPerPage >= proposalsAccepted.length}
          className="px-4 py-2 bg-gray-200 rounded disabled:opacity-50"
        >
          Next
        </button>
